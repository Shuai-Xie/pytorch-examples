...
from torch.nn.modules.utils import _single, _pair, _triple  # 通过 repeat 函数 将 single -> tuple
...
class _ConvNd(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        if transposed:
            ...
            self.weight = Parameter(torch.Tensor(
                in_channels, out_channels // groups, *kernel_size))  # * 拆解 kernel_size 适合 Nd 多维度
        else:
            # conv
            self.weight = Parameter(torch.Tensor(
    ...
    def reset_parameters(self):
        # 采用默认 nonlinearity='leaky_relu' 初始化 a^2 = 5
        init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        if self.bias is not None:
            fan_in, _ = init._calculate_fan_in_and_fan_out(self.weight)
            bound = 1 / math.sqrt(fan_in)  # gain=1, 没有√3
