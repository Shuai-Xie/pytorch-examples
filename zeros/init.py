...
def calculate_gain(nonlinearity, param=None):
    ...
    """
    # 提供对 不同非线性函数 推荐的信息增益
    linear_fns = ['linear', 'conv1d', 'conv2d', 'conv3d', 'conv_transpose1d', 'conv_transpose2d', 'conv_transpose3d']  # 1
    if nonlinearity in linear_fns or nonlinearity == 'sigmoid':  # 1
        return 1
...
def _calculate_fan_in_and_fan_out(tensor):
    ...
    if dimensions == 2:  # Linear
        ...
        if tensor.dim() > 2:
            receptive_field_size = tensor[0][0].numel()  # img2col, 计算 kernel_size numel
